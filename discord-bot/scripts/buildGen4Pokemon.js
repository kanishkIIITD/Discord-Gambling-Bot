...
async function getGen4Species() {
  console.log('Fetching Gen 4 species...');
...
async function buildGen4PokemonData() {
  try {
    console.log('Starting Gen 4 Pokémon data build...');
    ...
    // Build Gen 4 Pokémon data
    const gen4Species = await getGen4Species();
    console.log(`Found ${gen4Species.length} Gen 4 species`);
    
    const gen4PokemonData = {};
    const evolutionChains = new Map();
    for (const species of gen4Species) {
      try {
    ...
    console.log(`Successfully added ${Object.keys(gen4PokemonData).length} Gen 4 Pokémon to spawn rates.`);
    const gen4Count = Object.values(updatedSpawnRates).filter(p => p.gen === 4).length;
    console.log(`Total Gen 4 entries now: ${gen4Count}`);
  } catch (error) {
    console.error('Error building Gen 4 Pokémon data:', error);
    process.exit(1);
  }
}

if (require.main === module) {
  buildGen4PokemonData();
}

module.exports = { buildGen4PokemonData };
