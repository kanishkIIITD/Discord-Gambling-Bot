              ...
              {leaderboard.length > 0 ? (
                leaderboard.map((player, index) => (
                  <tr key={player.discordId} className={`hover:bg-primary/5 ${player.discordId === user?.discordId ? 'bg-primary/20' : ''}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-primary tracking-wide text-center">{player.maxWinStreak}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-yellow-500 tracking-wide text-center">{player.currentWinStreak}</td>
                  </tr>
                ))
              ) : (
                <tr>
                  <td colSpan="4" className="px-6 py-4 text-center text-sm text-text-secondary">No win streaks found.</td>
                </tr>
              )}
            </tbody>
          </table>
